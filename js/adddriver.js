...
var elementsCountDrivers = 0
      function addDriver(){
        ...
        let dnValue = document.createTextNode("ID: "+documentNumberValue)
        ...
        let flags = []
        Array.from(document.getElementById("countryList").selectedOptions).forEach(e => {
          let countryFlag = document.createElement("span")
          flags.push(e)
          countryFlag.className = "fi fi-"+e.value.toLowerCase()
        ...
        form.reset()
        // Values to modify modal
        document.getElementById('drivernamemodify').value = this.driverName
        document.getElementById('documentnumbermodify').value = this.documentNumberValue
      }